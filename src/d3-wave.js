...
export default class WaveGraph {
    constructor(svg) {
        ...
        this.rowRenderers = [
            ...
            new RowRendererBits(this),  // have own FORMATTERS and DEFAULT_FORMAT
            new RowRendererEnum(this),  // not use
            new RowRendererLabel(this), // not use
            new RowRendererStruct(this),
            new RowRendererArray(this), // have own FORMATTERS and DEFAULT_FORMAT
        ];
        this.timeZoom = null;
    ...
    setZoom() {
        ...
        console.log('[setZoom]this.xRange: %o', timeRange);
        ...
        // console.log('[setZoom]this.timeZoom: %o', timeZoom);
    }
    zoomed(ev) {
        console.log('[zoomed]');
        if (!this.xaxisG)
    ...
    drawYHelpLine() {
        ...
        function moveVerticalHelpLine(ev) {
            ...
            var xPos = ev.clientX - graph.sizes.margin.left; //x position within the element.
            if (xPos < 0) { xPos = 0; }
            svg.select('.vertical-help-line')
                .attr('transform', function () {
                    return 'translate(' + xPos + ',0)';
                })
                .attr('y2', graph.sizes.height).attr('debug-xPos', xPos)
                .attr('debug-ev.clientX', ev.clientX)
                .attr('debug-boundingRect.left', boundingRect.left)
                .attr('debug-graph.size.margin.left', graph.sizes.margin.left)
                .attr('delta', ev.clientX - boundingRect.left - graph.sizes.margin.left);
        }
    ...
    draw() {
        ...
        function renderWaveRows(selection) {
            ...
            selection.each(function (d) {
                ...
                // console.log('signalType[renderWaveRows]: ');
    ...
    bindData(_signalData) {
        ...
        // new findRendererAndDiscoverMaxT
        // function findRendererAndDiscoverMaxT(d) {
        //     var dData = d.signal;
        //     for (let i = 0; i < dData.length; ++i) {
        //         let signal = dData[i];
        //         let singalT = undefined;
        //         if (signal.wave.length)
        //             singalT = signal.wave[signal.wave.length - 1][0];

        //         if (typeof singalT == "string") {
        //             maxT = Math.max(maxT, parseFloat(singalT));
        //         } else if(typeof singalT == 'number') {
        //             maxT = Math.max(maxT, singalT);
        //         }
                
        //         console.log('signal: %o', signal);
        //         console.log('maxT: %f', maxT);
        //         for (let j = 0; j < rowRenderers.length; ++j) {
        //             let renderer = rowRenderers[j];
        //             console.log("renderer: %o", renderer);
        //             if (renderer.select(signal)) {
        //                 let formatter = renderer.DEFAULT_FORMAT;
        //                 signal.formatter = formatter;
        //                 signal.renderer = renderer;
        //                 break;
        //             }
        //         }
        //         if (!signal.renderer) {
        //             throw new Error('None of installed renderers supports signal:' + signal);
        //         }
        //     }
        // }
        ...
        function findRendererAndDiscoverMaxT(d) {
            ...
            for (var i = 0; i < rowRenderers.length; i++) {
                ...
                if (renderer.select(signalType)) {
                    ...
                    console.log('formatter second');
                    console.log(signalType.formatter);
                    break;
                }
            }
        ...
        console.log('maxT[bindData]: %f', maxT);
        console.log('this.allData[bindData]: %o', this.allData);

        var sizes = this.sizes;
        this.xRange[1] = sizes.row.range[1] = maxT;
        this.setZoom();
        console.log('after this.setZoom[binData]');
        var ROW_Y = sizes.row.height + sizes.row.ypadding;
        var graph = this;
        console.log('this.treelist[binData]: %o', this.treelist);
        if (!this.treelist) {
            console.log('(!)this.treelist[binData]');
            console.log('ROW_Y: %f, this.labelContextMenu: %o', ROW_Y, this.labelContextMenu);
            this.treelist = new TreeList(ROW_Y, this.labelContextMenu)
                .onChange(function (selection) {
                    console.log('TreeList onChange[binData]: %o', selection);
                    graph.data = selection.map((d) => { return d.data; }); // very important!!!
                    graph.draw();
                });
        }
        this.setSizes();
        console.log('after this.setSizes[binData]');
        this.treelist.data(this.allData);
        console.log('this.data[binData]: %o', this.data);
    }
}
